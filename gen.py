...
def deploy(name):   # build pages at the moment?
	print('Deploying %s\n'%(name))
	os.system("git config --global push.default matching && cd {path} && echo {pagesURL} > ./CNAME && git config user.email '{Email}' &&  git config user.name '{Account}' && git add . && git commit -m 'Update Pages' && git checkout -b gh-pages && git push && git push origin gh-pages -f > secret.txt ".format(
